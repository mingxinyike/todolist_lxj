var f = function() {
    ...
    var util = {
        ...
        get: function(callback) {
            _j._$request(url, {
                ...
                onload:function(data){
                    ...
                    callback.call(app, JSON.parse(data));
                },
                onerror:function(_error){
                    // 异常处理
        ...
        toggleAll: function(sign) {
            ...
            _j._$request(url, {
                ...
                onerror:function(_error){
                    // 异常处理
                    console.log("toggleAll error");
                }
            });
        },
        update: function(oldId, completed, title) {
            var obj = {
                "oldId": oldId,
                "completed": completed,
                "title": title
            ...
            _j._$request(url, {
                ...
                type:'binary',
                method:'POST',
                data:JSON.stringify({"newObj":obj}),
                timeout:3000,
                onload:function(data){
                    // 正常回调处理
                    console.log("POST successfully");
    ...
    app._destory = function(ev) {
        ...
        if (ev.target.tagName==="BUTTON") {
            var index = this._getIndexFromElem(ev.target);
            var oldId = this.todos[index].id;
    ...
    app._update = function(ev) {
        ...
        if (ev.target.className === "edit") {
            if ( ev.target.value != "") {
                var index = this._getIndexFromElem(ev.target);
                var oldId = this.todos[index].id;
                this.todos[index].title = ev.target.value;
                util.update(oldId, this.todos[index].completed, ev.target.value);
            }
            this.render(); 
        }
    ...
    app._toggle = function(ev) {
        ...
        if(ev.target.className === "toggle") {
            ...
            util.update(oldId, this.todos[index].completed, this.todos[index].title);
            this.render();
        }
...
NEJ.define(['{lib}util/template/tpl.js', '{lib}util/chain/chainable.js', '{lib}util/ajax/xdr'], f);
